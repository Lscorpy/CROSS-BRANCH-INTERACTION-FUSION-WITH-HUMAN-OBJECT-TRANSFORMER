...
DATA_PATH  = r"D:\coding\CROSS-BRANCH INTERACTION FUSION WITH HUMAN-OBJECT TRANSFORMER\test\gun_violence (61)_frame_00002.jpg"
WEIGHTS    = r"D:\coding\FYP_Project\Merge\checkpoints\DUAL_cross_branch_model\best.pth"
OUTPUT_DIR = r"D:\coding\CBIF_HOTR\three_stage_out"

SHOW_PLOTS = True


 
def select_image_file(initial_dir=None, title="Select an input image"):
    """
    Opens a simple native file-picker dialog (via tkinter, which ships with
    the standard Python install) so the user can browse to and choose the
    image to run inference on, instead of hard-coding DATA_PATH.
 
    Returns the chosen path as a string, or None if the dialog was
...
def run_three_stage(
    ...
    os.makedirs(output_dir, exist_ok=True)
     ...
     # If no image path was supplied, pop up a simple file-picker so the user
    # can browse to and choose the image interactively.
    if not image_path:
        image_path = select_image_file()
        if not image_path:
            raise ValueError("No image selected — aborting.")
...
if __name__ == "__main__":
    ...
    chosen_path = select_image_file(initial_dir=os.path.dirname(DATA_PATH))
    image_path_to_use = chosen_path or DATA_PATH
